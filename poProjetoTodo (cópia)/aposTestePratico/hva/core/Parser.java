 ...
 * 
 * public class Hotel {
 * public void registerAnimal(animalId, name, habitatId, speciesId) throws
 * OneOrMoreCoreExceptions { ... }
 * public void registerSpecies(speciesId, name) throws OneOrMoreCoreExceptions {
 * ... }
 * public void registerEmployee(employeeId, name, empType) throws
 * OneOrMoreCoreExceptions { ... }
 * public void addResponsibility(employeeId, responsibility) throws
 * OneOrMoreCoreExceptions { ... }
 * public void registerVaccine(vaccineId, name, String[] speciesIds) throws
 * someCoreExceptionsOneOrMoreCoreExceptions { ... }
 * public void createTree(TreeId, name, type, age, diff) throws
 * OneOrMoreCoreExceptions { ... }
 * public Habitat registerHabitat(habitatId, name, area) throws
 * OneOrMoreCoreExceptions { ... }
 * 
 * Note-se que esta funcionalidade pode ser utilizada na concretização de alguns
 * dos comandos.
 * Caso Hotel não tenha esta funcionalidade, então deverão substituir a
 * invocação destes métodos
 * na classe Parser por uma ou mais linhas com uma funcionalidade semelhante.
 * Cada um destes métodos pode lançar uma ou mais excepções que irão
 * corresponder aos erros que
 * podem acontecer ao nível do domínio surante a concretização da funcionalidade
 * em causa.
 **/
  ...
  private void parseHabitat(String[] components) throws UnrecognizedEntryException {
    try {
      ...
      if (components.length == 4) {
        _hotel.registerHabitat(id, name, area, null); // Passa null para responsabilidades
    } else if (components.length == 5) {
        String[] trees = components[4].split(",");
        List<String> tree = new ArrayList<>();
        tree.addAll(Arrays.asList(trees));
  ...
  private void parseEmployee(String[] components, String empType) throws UnrecognizedEntryException {
    try {
      ...
      if (components.length == 3) {
            _hotel.registerEmployee(id, name, empType, null); // Passa null para responsabilidades
        } else if (components.length == 4) {
            String[] responsibilities = components[3].split(",");
            List<String> responsability = new ArrayList<>();
            responsability.addAll(Arrays.asList(responsibilities));
            _hotel.registerEmployee(id, name, empType, responsability);
        //_hotel.addResponsibility(components[1], empType);
      }
    } catch (Exception e) {
      throw new UnrecognizedEntryException("Invalid entry: " + e);
    }
...
/**
 * Nota: O bloco catch presente nos vários métodos parse desta classe deverá ter
 * em conta
 * as várias excepções que podem acontecer no contexto do bloco try em questão.
 * Estas excepções do domínio têm que ser definidas por cada grupo e devem
